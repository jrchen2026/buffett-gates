...
5. **季检闭环**：快照留下四闸读数 + 退出信号表；持有阶段每季重读仪表，对事实/价格/认知三层报告位移与归因，画四象限轨迹——是体检，不是审判
...
A Buffett-style assay bench for investment candidates you have already picked (listed companies only). One admission check (worth assaying at all: a cash-machine look, or a mispriced look), three gates traceable to specific shareholder-letter years (anchor / moat / circle-and-margin / Mr. Market) — the anchor includes a reverse check (what growth does the current price imply, and is that absurd?), a mandatory adversarial pass when the readings point to betting money (an independent "credentialed bear" sub-agent, per the 2012 letter, that only cuts readings down and names three observable ways the investment dies), a four-quadrant verdict where the quadrant dictates the action, and a second stage — quarterly review — that reads the displacement of facts, price, and your own judgment against the last snapshot. Conclusions are structurally forced — no fence-sitting allowed.
